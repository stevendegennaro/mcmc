...
void loadGirardi(char *path, int filterSet){
  ...
  if(loaded && verbose) {printf("Girardi models already loaded.\n"); return;}
  else loaded = 1;

  if(filterSet != UBVRIJHK && filterSet != ACS){
...

{
  ...
  //printf("%f %f %f\n",newFeH,newY,newAge);
	...
	if(newAge < 7.80) {
		if(verbose) printf("\n Requested age (%f) too young. (gGirMag.c)",newAge);
		return 0.0;
	}
	if(newAge > 10.25) {
		if(verbose) printf("\n Requested age (%f) too old. (gGirMag.c)",newAge);
		return 0.0;
	}
	if(newFeH < gFeH[0]){
		if(verbose) printf("\n Requested FeH (%f) too low. (gGirMag.c)",newFeH);
		return 0.0;
	}
	if(newFeH > gFeH[N_GIR_Z - 1]){
		if(verbose) printf("\n Requested FeH (%f) too high. (gGirMag.c)",newFeH);
		return 0.0;
	}
