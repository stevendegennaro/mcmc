...
void loadChaboyer(char *path, int filterSet){
  ...
  if(loaded && verbose) {printf("Chaboyer models already loaded.\n"); return;}
  else loaded = 1;
...
double deriveChabAgbTip(double newFeH, double newY, double newLogAge){
  ...
  if(newLogAge < cLogAge[0][0][0]) {
    if(verbose) printf("\n Requested age (%.3f) too young. (gChabMag.c)",newLogAge);
    return 0.0;
  }
  ...
  for(m=0;m<neweep;m++){
    ...
    if(m>0){
      ...
      while(isochrone.mass[n] < isochrone.mass[n-1] && n>0){
        swapGlobalEntries(&isochrone,n,useFilt);  //Function defined in structures.c
        n--;
      } 
    }
  }
  ...
  // printf("turnoff mass = %f\n",isochrone.AgbTurnoffMass);
